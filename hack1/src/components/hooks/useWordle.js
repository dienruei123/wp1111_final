...
const useWordle = (solution) => {
  ...
  const handleEnter = () => {
    ...
    for (let i = 0; i < guessList.length; i++) {
      guessResult[turn].push({});
      guessResult[turn][i].char = guessList[i];
      if (solList[i] === guessList[i]) {
        used[i] = true;
    ...
    for (let i = 0; i < guessList.length; i++) {
      if (guessResult[turn][i].color === "green") {
        usedChars_[guessResult[turn][i].char] = "green";
      }
    }
    setUsedChars(usedChars_);
  };
